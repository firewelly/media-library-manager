...
class FileUtils:
    ...
    @staticmethod
    def copy_file_with_progress(src: str, dst: str, callback=None, buffer_size: int = 1024 * 1024) -> bool:
        """复制文件并支持进度回调
    ...
    @staticmethod
    def move_file_with_progress(src: str, dst: str, callback=None) -> bool:
        """移动文件并支持进度回调"""
        try:
            # 确保目标目录存在
            os.makedirs(os.path.dirname(dst), exist_ok=True)
            ...
            try:
                ...
                # 跨文件系统，需要复制后删除
                if FileUtils.copy_file_with_progress(src, dst, callback):
                    try:
                        os.remove(src)
                        return True
                    except Exception as e:
                        logger.error(f"删除源文件失败 {src}: {e}")
                        # 复制成功但删除失败，仍视为移动成功（为了数据安全）
                        return True
                return False
        except Exception as e:
